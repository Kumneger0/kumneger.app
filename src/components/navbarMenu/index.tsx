...
import { Home, BlogBlog } from '@/routes'
...
function NavBarMenu({ className }: { className: string }) {
  ...
  const openModalRef = useRef<ElementRef<typeof LoginModal>>(null)

  return (
    <div className={cn("flex w-1/2 justify-center items-center ", className)}>
      <div>
        <Button variant={"link"}>
          <Home.Link className="text-xl">
      ...
      <div>
        <Button variant={"link"}>
          <BlogBlog.Link className="text-xl">
            Blog
          </BlogBlog.Link>
        </Button>
      </div>
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px] bg-white">
        ...
        <div className="grid  text-black gap-4 py-6">
          {providers?.github ? (
            <Button
              onClick={() => {
              ...
              variant="outline"
            >
              <GithubIcon className="mr-2 text-black h-5 w-5" />
              Sign in with GitHub
            </Button>
          ) : null}
        </div>
